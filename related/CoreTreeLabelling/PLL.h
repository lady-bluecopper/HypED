...
public:
	SN();
	SN(int id, long long val);
	int id;
	long long val;
	bool operator < (const SN& v) const;
};

template<int kNumBitParallelRoots = 16>//50>//50
class PrunedLandmarkLabeling {
 public:
  ...
  PrunedLandmarkLabeling()
      : num_v_(0), index_(NULL), time_load_(0), time_indexing_(0), MAXDIS(0), MAXMOV(0), MASK(0) {}
...
bool SN::operator < (const SN& v) const {
	if( val > v.val ) return true;
	if( val < v.val ) return false;
	return id < v.id;
}

SN::SN(int id, long long val) {
...
::ConstructIndex(const std::vector<std::pair<int, int> > &es) {
  ...
  MAXDIS = 2; MAXMOV = 1;
  while( MAX_INT / ((V+1) * 2) >= MAXDIS ) {MAXDIS *= 2;++MAXMOV;}
  MASK = MAXDIS - 1;
  //printf( "V=%d,MAXMOV=%d\n", V, MAXMOV );
  ...
  std::vector<int> inv(V);  // new label -> old label
  {
	...
	SN *f = new SN[V];
	for( int i = 0; i < V; ++i )
		f[i].id = i, f[i].val = (long long) adj[i].size();
	sort(f, f + V);
	for(int i = 0; i < V; ++i) {rank[f[i].id] = i;inv[i] = f[i].id;}
	delete[] f;
  ...
  printf( "bit_parallel_labeling time = %0.3lf secs\n", omp_get_wtime() - tt );
  printf( "pruned labeling...\n" );
  //
  // Pruned labeling
  //
  {
    ...
    for (int v = 0; v < V; ++v) {
      ...
      for (int i = 0; i < k; ++i) index_[inv[v]].spt_v[i] = (((unsigned) tmp_idx[v].first[i])<<MAXMOV)|tmp_idx[v].second[i];
...
::QueryDistance(int v, int w) {
  ...
  for (int i1 = 0, i2 = 0; ; ) {
    int v1 = idx_v.spt_v[i1]>>MAXMOV, v2 = idx_w.spt_v[i2]>>MAXMOV;
    if (v1 == v2) {
      if (v1 == num_v_) break;  // Sentinel
      int td = (idx_v.spt_v[i1]&MASK) + (idx_w.spt_v[i2]&MASK);
      if (td < d) d = td;
      ++i1;
      ++i2;
    } else {
...
::LoadIndex(std::istream &ifs) {
  ...
  ifs.read((char*)&MAXMOV,sizeof(unsigned));
...
::StoreIndex(std::ostream &ofs) {
  ...
  for (int v = 0; v < num_v_; ++v) {
    ...
    int32_t s;
    for (s = 1; idx.spt_v[s - 1] != (((unsigned)num_v)<<MAXMOV) + INF8; ++s) continue;  // Find the sentinel
  ...
  ofs.write((const char*)&MAXMOV, sizeof(MAXMOV));
...
::PrintStatistics() {
  ...
  double s = 0.0, t = 0.0;
  for (int v = 0; v < num_v_; ++v) {
    for (int i = 0; (index_[v].spt_v[i]>>MAXMOV) != uint32_t(num_v_); ++i) {
      ++s; t += sizeof(unsigned);
    }
    t += (8*2+1) * kNumBitParallelRoots;
  }
  s /= num_v_;

  std::cout << "bit-parallel label size: "   << kNumBitParallelRoots << std::endl;
  std::cout << "average normal label size: " << s << std::endl;
  std::cout << "index size: " << t / (1024.0*1024.0) << " MB" << std::endl;
}
